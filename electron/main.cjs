...
// Whether this edition ships an atlas. The renderer's own copy lives in
// src/runtime/edition.js; the main process cannot import an ES module, so the
// value is repeated here and a test (src/runtime/edition.test.js) fails the
// build if the two ever disagree. A repeated constant nobody checks is how a
// game ends up downloading 289 MB of tiles it no longer draws.
const EDITION_HAS_MAP = false;
...
const missingAssets = () => {
  // An edition with no map needs no cartography. This one call decided whether a
  // new player waited on a 289 MB download — 106 MB of region geometry, 63 MB of
  // country geometry and two 55 MB seeds — before the first turn of a game
  // played in a newspaper, whose polity owns half a square kilometre and never
  // moves a frontier. The names it actually wanted now ship as 8 kB
  // (scripts/extract-country-names.mjs). One guarded early return, so switching
  // the map back on in src/runtime/edition.js brings the download back with it.
  if (!EDITION_HAS_MAP) return [];
  let manifest;
  try {
