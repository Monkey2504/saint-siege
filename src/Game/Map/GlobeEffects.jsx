...
const GlobeEffects = ({ active }) => {
  ...
  useEffect(() => {
    ...
    // A globe nobody is looking at still costs a full MapLibre re-render, a
    // starfield redraw and a lighting pass several times a second, for as long
    // as the app is open. Chromium throttles rAF in a hidden TAB, but an
    // Electron window that is merely behind another one is not hidden — so
    // this stops the loop on blur as well, and restarts it (with the rotation
    // clock reset, so the globe doesn't jump by the whole away-interval).
    let paused = typeof document !== "undefined" && document.hidden;
    const tick = (now) => {
      if (disposed || contextLost || paused || !mapInstance.style) return;
    ...
    const resumeLoop = () => {
      if (!paused || disposed || contextLost) return;
      paused = false;
      lastRotateTick = performance.now();
      frameId = requestAnimationFrame(tick);
    };
    const handleVisibility = () => (document.hidden ? pauseLoop() : resumeLoop());
    ...
    return () => {
      ...
      window.removeEventListener("blur", pauseLoop);
