...
import { mergeExtraRegionsIntoSeed } from "../../runtime/extraRegions.js";
...
const WorldMap = ({ isGlobe = false }) => {
  ...
  const [rawRegionSeed, setRegionSeed] = useState(null);
  // The scenario's seed plus the territory THIS game adds (world.extraRegions —
  // e.g. the Vatican enclave): same shape, so everything below is unchanged.
  const regionSeed = useMemo(() => mergeExtraRegionsIntoSeed(rawRegionSeed, extraRegions), [rawRegionSeed, extraRegions]);
  ...
  const resolveOwnerRgbUncached = useCallback(
    (rawOwner) => {
  ...
  // Memoized per palette identity, and declared AFTER what it calls: without
  // the cache every repaint after a jump re-ran the fold-key scan above —
  // Object.entries over the whole palette AND the polity registry, normalized
  // per entry — once per region, thousands of times, for an answer that cannot
  // change until the palette itself does.
  const ownerRgbCache = useMemo(() => new Map(), [colorMap, polityOverrides]);
  const resolveOwnerRgb = useCallback(
    (rawOwner) => {
      if (!rawOwner) return null;
      if (ownerRgbCache.has(rawOwner)) return ownerRgbCache.get(rawOwner);
      const computed = resolveOwnerRgbUncached(rawOwner);
      ownerRgbCache.set(rawOwner, computed);
      return computed;
    },
    [ownerRgbCache, resolveOwnerRgbUncached],
  );
