...
export function useWorldState() {
  ...
  // The game's own authored regions (runtime/extraRegions.js) — content-compared
  // so the map's seed merge only re-runs when territory actually changes.
  const extraRegions =
    prev && JSON.stringify(prev.extraRegions) === JSON.stringify(state?.extraRegions ?? null)
      ? prev.extraRegions
      : state?.extraRegions ?? null;
  ...
  const output =
    ...
    prev.extraRegions === derived.extraRegions &&
