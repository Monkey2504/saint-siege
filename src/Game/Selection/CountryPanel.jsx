...
const CountryInfoPanel = () => {
    ...
    return createPortal(
        ...
        {country.flagUrl && !flagFailed ? (
            ...
            <span style={{ fontSize: "var(--oh-t-lg)" }}>{country.flagEmoji}</span>
        ) : null}
        <span style={{ flex: 1, fontSize: "var(--oh-t-md)", fontWeight: 800, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
        {country.name}
        </span>
        <button
        type="button"
        onClick={() => setCountry(null)}
        style={{ background: "none", border: "none", color: "var(--oh-text)", cursor: "pointer", fontSize: "var(--oh-t-md)", lineHeight: 1, padding: "0.2rem" }}
        >
        {"✕"}
        </button>
        </div>
        ...
        <div style={{ fontSize: "var(--oh-t-base)", fontWeight: 800 }}>Related Events</div>
        <div style={{ color: "var(--oh-text-dim)", fontSize: "var(--oh-t-xs)" }}>{filteredEvents.length} shown</div>
        </div>
        ...
        style={{ background: "var(--oh-plate-2)", border: "1px solid var(--oh-line)", borderRadius: 8, color: "var(--oh-text)", flex: 1, fontSize: "var(--oh-t-xs)", outline: "none", padding: "0.55rem 0.7rem" }}
        />
        <button
        type="button"
        onClick={() => setFilterIndex((filterIndex + 1) % FILTER_MODES.length)}
        title="Filter by importance"
        style={{ ...footerButtonStyle, borderRadius: 8, flex: "none", fontSize: "var(--oh-t-xs)", padding: "0.45rem 0.7rem" }}
        >
        ...
        {filteredEvents.length === 0 ? (
            <div style={{ color: "var(--oh-text-dim)", fontSize: "var(--oh-t-xs)", padding: "0.3rem 0 0.4rem" }}>
            ...
            {filteredEvents.slice(0, 30).map((event) => (
                <div key={event.id} style={{ background: "var(--oh-plate-2)", border: "1px solid var(--oh-line)", borderRadius: 10, padding: "0.55rem 0.7rem" }}>
                <div style={{ alignItems: "baseline", display: "flex", gap: "0.5rem", justifyContent: "space-between" }}>
                <span style={{ fontSize: "var(--oh-t-xs)", fontWeight: 700 }}>{event.title}</span>
                <span style={{ color: "var(--oh-text-dim)", flexShrink: 0, fontSize: "var(--oh-t-2xs)" }}>{event.date}</span>
                </div>
                {event.description && (
                    <div style={{ color: "var(--oh-text)", fontSize: "var(--oh-t-xs)", lineHeight: 1.5, marginTop: "0.2rem" }}>
        ...
        <div style={{ fontSize: "var(--oh-t-base)", fontWeight: 800, marginTop: "0.5rem" }}>Details</div>
        <div style={{ display: "grid", gap: "0.8rem", gridTemplateColumns: "minmax(0, 1fr) minmax(0, 1.4fr)" }}>
        <div>
        <div style={{ fontSize: "var(--oh-t-xs)", fontWeight: 700, marginBottom: "0.35rem" }}>Alternative Names</div>
        {aliases.length === 0 ? (
            <div style={{ color: "var(--oh-text-dim)", fontSize: "var(--oh-t-xs)" }}>None</div>
        ) : (
        ...
        <div>
        <div style={{ fontSize: "var(--oh-t-xs)", fontWeight: 700, marginBottom: "0.35rem" }}>Regions Owned ({regions.length})</div>
        {regions.length === 0 ? (
            <div style={{ color: "var(--oh-text-dim)", fontSize: "var(--oh-t-xs)" }}>None</div>
        ) : (
            <div style={{ display: "flex", flexWrap: "wrap", gap: "0.3rem" }}>
        ...
        {report !== null && (
            <div style={{ background: "var(--oh-plate-2)", border: "1px solid var(--oh-line)", borderRadius: 10, marginTop: "0.6rem", padding: "0.7rem 0.8rem" }}>
            <div style={{ fontSize: "var(--oh-t-xs)", fontWeight: 700, marginBottom: "0.3rem" }}>Advisor Report</div>
            {report === "loading" ? (
                <div style={{ color: "var(--oh-text-dim)", fontSize: "var(--oh-t-xs)" }}>Preparing the report…</div>
            ) : report?.error ? (
                <div style={{ color: "var(--oh-alert)", fontSize: "var(--oh-t-xs)" }}>{report.error}</div>
            ) : (
                <div className="timeline-markdown" style={{ color: "var(--oh-text-strong)", fontSize: "var(--oh-t-xs)", lineHeight: 1.55 }}>
        ...
        {/* Footer */}
        <div style={{ borderTop: "1px solid var(--oh-line)", display: "flex", gap: "0.6rem", padding: "0.8rem 1.1rem" }}>
        <button type="button" onClick={runAdvisorReport} style={footerButtonStyle}>
        Advisor Report
        </button>
        <button type="button" onClick={openDiplomacy} style={{ ...footerButtonStyle, background: "var(--oh-accent-soft)", border: "1px solid var(--oh-line)" }}>
        Open Diplomacy
        </button>
        </div>
        </div>,
