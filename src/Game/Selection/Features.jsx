...
const DetailRow = ({ label, value }) => (
  <div style={{ display: "flex", justifyContent: "space-between", gap: "10px", fontSize: "var(--oh-t-2xs)", color: "var(--oh-text)", marginTop: "3px" }}>
    <span style={{ flexShrink: 0 }}>{label}</span>
    <span style={{ color: "var(--oh-text-strong)", textAlign: "right", wordBreak: "break-word" }}>{value}</span>
  </div>
);
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "9px", padding: "10px 12px 8px" }}>
          <span style={{ fontSize: "var(--oh-t-lg)", lineHeight: 1 }}>
            {isCity ? (feature.capital === "primary" ? "⭐" : "🏙") : emojiForKind(feature.kind)}
          </span>
          <div style={{ minWidth: 0, flex: 1 }}>
            <div style={{ fontWeight: 700, fontSize: "var(--oh-t-xs)", wordBreak: "break-word" }}>{feature.name}</div>
        ...
        <div style={{ padding: "0 12px 10px" }}>
          ...
          {feature.note ? (
            <div style={{ marginTop: "8px", fontSize: "var(--oh-t-2xs)", lineHeight: 1.45, color: "var(--oh-text)" }}>
              {feature.note}
            </div>
          ) : null}
        </div>
