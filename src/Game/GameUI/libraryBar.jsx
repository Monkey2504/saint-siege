...
import { CHURCH_START_DATE, applyChurchPreset } from "../../runtime/churchPreset.js";
...
const buildGameEditorState = (details) => {
  ...
  return {
    accentColor: gameMeta.accentColor ?? "var(--oh-accent)",
    country: game.country ?? "",
...
const PromptSectionEditor = ({
  ...
  return (
    ...
    >
        ...
        {PROMPT_SECTION_DEFINITIONS.map((section) => (
          <button
            ...
            style={{
              ...actionButtonStyle,
              background:
                section.key === currentSection.key ? "var(--oh-accent-soft)" : "var(--oh-plate-2)",
              borderColor:
                section.key === currentSection.key ? "var(--oh-accent-soft)" : "var(--oh-line)",
              minHeight: "2rem",
              padding: "0 0.8rem",
            }}
            type="button"
      ...
      <div style={{ color: "var(--oh-text-dim)", fontSize: "var(--oh-t-xs)", marginBottom: "0.75rem" }}>
  ...
  return (
    ...
    >
      ...
      {/* The picture is a band across the top, the way a paper runs a photograph
          ...
          on top — unreadable once the title went from white to ink. */}
      <div
        style={{
          background:
      ...
      >
        <div>
          <div style={{ alignItems: "center", display: "flex", justifyContent: "space-between" }}>
            <div style={{ alignItems: "center", display: "flex", flexWrap: "wrap", gap: "0.45rem" }}>
              <span
                style={{
                  background: selected ? `${scenario.accentColor}66` : "var(--oh-plate-2)",
                  border: "1px solid var(--oh-line)",
                  borderRadius: "999px",
                  color: "var(--oh-text-strong)",
                  display: "inline-flex",
        ...
        <div>
          <div style={{ color: "var(--oh-text)", fontSize: "var(--oh-t-xs)", marginBottom: "0.7rem" }}>
            {scenario.subtitle}
          </div>
...
const GameCard = ({ active, game, onActivate, onArchive, onClone, onEdit }) => {
  ...
  return (
    ...
    >
      {/* Same treatment as the scenario card: the picture is a band on top and
          the words sit on paper under it, not on a darkened photograph. */}
      <div
        style={{
          background:
      ...
      >
        <div>
          <div style={{ alignItems: "center", display: "flex", justifyContent: "space-between" }}>
            <span
              style={{
                background: active ? `${game.accentColor}66` : "var(--oh-plate-2)",
                border: "1px solid var(--oh-line)",
                borderRadius: "999px",
                color: "var(--oh-text-strong)",
                display: "inline-flex",
          ...
          <div style={{ marginTop: "2rem" }}>
            <div style={{ color: "var(--oh-text-strong)", fontSize: "var(--oh-t-lg)", fontWeight: 800, letterSpacing: "-0.03em" }}>
              {game.name}
            </div>
        ...
        <div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "0.55rem" }}>
            <button
              ...
              style={{
                ...actionButtonStyle,
                background: active ? "var(--oh-plate-2)" : `${game.accentColor}cc`,
                borderColor: active ? "var(--oh-line)" : `${game.accentColor}dd`,
                color: "var(--oh-text-strong)",
                flex: 1,
              }}
              type="button"
...
const MenuRow = ({ children, emptyText, title }) => (
  <div style={{ marginBottom: "1.7rem" }}>
    <div style={{ color: "var(--oh-text-strong)", fontSize: "var(--oh-t-base)", fontWeight: 800, letterSpacing: "-0.01em", marginBottom: "0.7rem" }}>
      {title}
    </div>
    {React.Children.count(children) > 0 ? (
      ...
      <div style={{ color: "var(--oh-text-dim)", fontSize: "var(--oh-t-xs)", padding: "0.4rem 0 0.6rem" }}>
...
const CreateScenarioTile = ({ busy, onCreate }) => (
  ...
  >
    <span aria-hidden="true" style={{ fontSize: "var(--oh-t-xl)", fontWeight: 300, lineHeight: 1 }}>+</span>
    <span style={{ fontSize: "var(--oh-t-sm)", fontWeight: 700 }}>Create Scenario</span>
  </button>
);
    ...
    {sections.map((sectionKey) => (
      <button
        ...
        style={{
          ...actionButtonStyle,
          background:
            currentSection === sectionKey ? "var(--oh-accent-soft)" : "var(--oh-plate-2)",
          borderColor:
            currentSection === sectionKey ? "var(--oh-accent-soft)" : "var(--oh-line)",
          minHeight: "2rem",
          padding: "0 0.8rem",
        }}
        type="button"
  ...
  return (
    ...
    >
      <div style={{ alignItems: "center", display: "flex", justifyContent: "space-between", marginBottom: "1rem" }}>
        ...
        <button
          onClick={onClose}
          style={{ ...actionButtonStyle, background: "var(--oh-plate-2)", minWidth: "2.35rem", padding: 0 }}
          type="button"
        >
          X
        </button>
                  ...
                  {UNIT_TYPES.map((unitType) => {
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...actionButtonStyle,
                          background: checked ? "var(--oh-accent-soft)" : "var(--oh-plate-2)",
                          borderColor: checked ? "var(--oh-accent-soft)" : "var(--oh-line)",
                          minHeight: "2rem",
                          padding: "0 0.7rem",
                        }}
                      >
        ...
        <div style={{ background: "var(--oh-plate-2)", border: "1px solid var(--oh-line)", borderRadius: "18px", marginBottom: "0.95rem", padding: "0.9rem" }}>
          <div style={{ display: "grid", gap: "0.7rem" }}>
            {Object.entries(kind === "scenario" ? scenarioAssetLabels : gameAssetLabels).map(([assetKey, label]) => {
              ...
              return (
                <div key={assetKey} style={{ alignItems: "center", background: "var(--oh-plate-2)", border: "1px solid var(--oh-line)", borderRadius: "14px", display: "flex", gap: "0.75rem", justifyContent: "space-between", padding: "0.72rem 0.78rem" }}>
                  ...
                  <div style={{ display: "flex", gap: "0.45rem" }}>
                    ...
                    <button
                      ...
                      style={{
                        ...actionButtonStyle,
                        background: "var(--oh-plate-2)",
                        color: hasOwnAsset ? "var(--oh-text-strong)" : "var(--oh-text-dim)",
                      }}
      ...
      {editorError && (
        <div style={{ background: "var(--oh-alert-soft)", border: "1px solid var(--oh-alert-soft)", borderRadius: "14px", color: "var(--oh-text-strong)", marginBottom: "0.9rem", padding: "0.8rem 0.9rem" }}>
          {editorError}
        </div>
      )}
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "0.55rem" }}>
        ...
        {kind === "scenario" && onOpenMapEditor && (
          <button
            ...
            style={{ ...actionButtonStyle, background: "var(--oh-accent-soft)", borderColor: "var(--oh-accent-soft)", color: "var(--oh-text-strong)", minWidth: "9rem" }}
            type="button"
          >
            🗺️ Open Map Editor
          </button>
...
const LibraryTopBar = () => {
  ...
  const startGameForFaction = async (scenario, faction, difficulty) => {
    ...
    try {
      ...
      const hexColor = /^#[0-9a-fA-F]{6}$/.test(faction.color) ? faction.color : "var(--oh-accent)";
      ...
      // The reforming-pope mode rides the same path as any faction, then merges
      // in the Church: its counter-powers, bodies, finances and starting intents.
      // Same read-merge-write discipline — applyChurchPreset returns a new world
      // built on the one createGame seeded, never a fresh one.
      let worldToSave = world;
      if (faction.preset === "church") {
        const gameDate = gameDetails?.data?.game?.gameDate || gameDetails?.data?.game?.startDate || "";
        worldToSave = applyChurchPreset(world, { date: gameDate, availableCountries: Object.values(COUNTRY_NAMES) });
      }

      // The reforming-pope mode is the Church AS IT IS — its accounts, its
      // College, its constitution are those of today — so it starts today,
      // not on the scenario's 2016 stock date.
      const datePatch = faction.preset === "church" ? { startDate: CHURCH_START_DATE, gameDate: CHURCH_START_DATE } : null;
      await saveGame(gameId, { world: worldToSave, gamePatch: { country: name, ...datePatch, ...(difficulty ? { difficulty } : null) } });
    ...
    <>
      ...
      {menuOpen && (
        <div
          style={{
            background:
              "radial-gradient(circle at 12% -4%, var(--oh-accent-soft), transparent 42%), " +
              "radial-gradient(circle at 88% 110%, var(--oh-accent-soft), transparent 46%), " +
              "linear-gradient(180deg, var(--oh-plate) 0%, var(--oh-plate-2) 100%)",
            color: "var(--oh-text-strong)",
            display: "flex",
            flexDirection: "column",
            fontFamily: "inherit",
            ...
            <div style={{ alignItems: "center", display: "flex", gap: "0.55rem", justifyContent: "center", justifySelf: "center" }}>
              ...
              {LIBRARY_TABS.map(([tab, label]) => (
                <button
                  ...
                  style={{
                    ...actionButtonStyle,
                    background: activeTab === tab ? "var(--oh-accent-soft)" : "var(--oh-plate-2)",
                    borderColor: activeTab === tab ? "var(--oh-accent-soft)" : "var(--oh-line)",
      ...
      {!loaded && (
        <div style={{ color: "var(--oh-text)", fontSize: "var(--oh-t-xs)", position: "fixed", right: "1.25rem", top: "4.35rem", zIndex: 10028 }}>
