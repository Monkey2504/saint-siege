...
const ScenarioCard = ({ post, busy, onImport, onSelect }) => (
  ...
  >
      ...
      <div style={{ minWidth: 0 }}>
        <div
          ...
          style={{
            ...
            color: post.official ? "var(--oh-accent)" : "var(--oh-text-strong)",
            fontSize: "var(--oh-t-sm)",
            fontWeight: 800,
        ...
        <div style={{ color: "var(--oh-text-dim)", fontSize: "var(--oh-t-2xs)" }}>
          {post.official && (
            <span style={{ background: "var(--oh-accent-soft)", border: "1px solid var(--oh-accent-soft)", borderRadius: "999px", color: "var(--oh-accent)", fontSize: "var(--oh-t-2xs)", fontWeight: 700, letterSpacing: "var(--oh-label-track)", marginRight: "0.35rem", padding: "0.08rem 0.4rem", textTransform: "var(--oh-label-case)" }}>
              ✓ Official
            </span>
          )}
    ...
    <div style={{ color: "var(--oh-text)", flex: 1, fontSize: "var(--oh-t-xs)", lineHeight: 1.5 }}>
    ...
    <div style={{ alignItems: "center", display: "flex", gap: "0.5rem" }}>
      ...
      <button
        ...
        style={{
          ...pillButton,
          minHeight: "1.8rem",
          background: post.bundleUrl ? "var(--oh-accent-soft)" : "var(--oh-plate-2)",
          borderColor: post.bundleUrl ? "var(--oh-accent-soft)" : "var(--oh-line)",
          color: post.bundleUrl ? "var(--oh-text-strong)" : "var(--oh-text-dim)",
...
const ScenarioRow = ({ title, posts, busyId, onImport, onSelect, emptyText, layout = "scroll" }) => (
  <div style={{ marginBottom: "1.15rem" }}>
    ...
    {posts.length === 0 ? (
      <div style={{ color: "var(--oh-text-dim)", fontSize: "var(--oh-t-xs)", padding: "0.3rem 0 0.6rem" }}>
...
const detailStat = { color: "var(--oh-text)", fontSize: "var(--oh-t-xs)" };
...
const StatusBanner = ({ notice, error }) => (
  <>
    {notice && (
      <div style={{ background: "var(--oh-grant-soft)", border: "1px solid var(--oh-grant-soft)", borderRadius: "12px", color: "var(--oh-grant)", fontSize: "var(--oh-t-xs)", marginBottom: "0.9rem", padding: "0.7rem 0.85rem" }}>
        {notice}
      </div>
    )}
    {error && (
      <div style={{ background: "var(--oh-alert-soft)", border: "1px solid var(--oh-alert-soft)", borderRadius: "12px", color: "var(--oh-text-strong)", fontSize: "var(--oh-t-xs)", marginBottom: "0.9rem", padding: "0.7rem 0.85rem" }}>
        {error}
      </div>
    )}
  </>
...
const ScenarioDetail = ({ post, busy, onImport, onBack, notice, error }) => (
  <div style={{ color: "var(--oh-text-strong)" }}>
    <button
      type="button"
      onClick={onBack}
    ...
    <div style={{ alignItems: "center", display: "flex", gap: "0.6rem", marginBottom: "0.3rem" }}>
      ...
      <h3 style={{ fontSize: "var(--oh-t-lg)", fontWeight: 800, margin: 0 }}>
        {post.pinned ? "📌 " : ""}{post.title}
      </h3>
    </div>
    <div style={{ color: "var(--oh-text-dim)", fontSize: "var(--oh-t-xs)", marginBottom: "0.9rem" }}>
      by {post.author} · {new Date(post.createdAt).toLocaleDateString()}
    </div>
...
const CommunityPanel = ({ fullPage = false, onImported }) => {
  ...
  return (
    ...
    <div style={{ color: "var(--oh-text-strong)", ...(fullPage ? {} : { maxHeight: "calc(100vh - 11rem)", overflowY: "auto", paddingRight: "0.2rem" }) }}>
      {selectedPost ? (
        <ScenarioDetail
          post={selectedPost}
        ...
        <>
          <div style={{ alignItems: "center", display: "flex", flexWrap: "wrap", gap: "0.5rem", marginBottom: "0.9rem" }}>
            ...
            <button
              ...
              style={{ ...pillButton, background: "var(--oh-accent-soft)", borderColor: "var(--oh-accent-soft)" }}
            >
              ⬆ Publish to Hub
            </button>
