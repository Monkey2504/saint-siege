...
const PolitySelect = ({ polities, value, onChange, placeholder = "Pick a country…" }) => (
    ...
    {polities.map((polity) => (
        <option key={polity.code} value={polity.code} style={{ color: "var(--oh-text-strong)" }}>
        {polity.name}
        </option>
    ))}
    </select>
...
const CheatsPanel = ({ open, onClose, onOpenForces }) => {
    ...
    const header = (title, subtitle) => (
        <div style={{ borderBottom: "1px solid var(--oh-line)", marginBottom: "0.7rem", paddingBottom: "0.6rem" }}>
        ...
        <div style={{ fontSize: "var(--oh-t-base)", fontWeight: 800 }}>{title}</div>
        </div>
        <button type="button" onClick={onClose} style={{ ...buttonStyle, padding: "0.25rem 0.55rem" }}>✕</button>
        </div>
        {subtitle && <div style={{ color: "var(--oh-text-dim)", fontSize: "var(--oh-t-xs)", marginTop: "0.2rem" }}>{subtitle}</div>}
        </div>
    );
    ...
    return (
        ...
        {!tool ? (
            ...
            {typeof onOpenForces === "function" && (
                ...
                <span style={{ color: "var(--oh-text-dim)", fontSize: "var(--oh-t-xs)", fontWeight: 500 }}>Open the Forces panel to spawn, move, and command units by hand.</span>
                </button>
            )}
            {TOOLS.map((entry) => (
                <button
                ...
                <span style={{ color: "var(--oh-text-dim)", fontSize: "var(--oh-t-xs)", fontWeight: 500 }}>{entry.subtitle}</span>
                </button>
            ))}
            </div>
            </>
        ...
        {status && !tool && (
            <div style={{ color: "var(--oh-text-dim)", fontSize: "var(--oh-t-xs)", marginTop: "0.6rem" }}>{status}</div>
        )}
        </div>
        </>
    );
...
const ToolView = ({ tool, header, busy, status, game, polities, refresh, runBusy, beginClickMode, endClickMode, setStatus }) => {
    ...
    const statusLine = status && (
        <div style={{ color: status.startsWith("Failed") ? "var(--oh-alert)" : "var(--oh-text-dim)", fontSize: "var(--oh-t-xs)", marginTop: "0.6rem" }}>
        {status}
        </div>
    );
    ...
    if (tool === "roll-back-turn") {
        ...
        return (
            ...
            {snapshots.map((snap, index) => {
                ...
                return (
                    <div key={snap.id} style={{ background: "var(--oh-plate-2)", border: "1px solid var(--oh-line)", borderRadius: 8, padding: "0.5rem 0.6rem" }}>
                    <div style={{ alignItems: "center", display: "flex", gap: "0.4rem", justifyContent: "space-between" }}>
                    <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: "var(--oh-t-xs)", fontWeight: 700 }}>Round {snap.round}{dateLabel ? ` · ${dateLabel}` : ""}</div>
    ...
    if (tool === "difficulty") {
        ...
        return (
            ...
            {DIFFICULTY_LEVELS.map((level) => (
                ...
                style={{
                    ...buttonStyle,
                    background: current === level.id ? "var(--oh-accent-soft)" : buttonStyle.background,
                    border: current === level.id ? "1px solid var(--oh-accent-soft)" : buttonStyle.border,
                    flexDirection: "column",
                    gap: "0.15rem",
    ...
    if (tool === "edit-country" || tool === "add-country") {
        ...
        return (
            ...
            <input style={{ ...inputStyle, width: "8rem" }} value={fields.color ?? ""} onChange={(event) => setFields({ ...fields, color: event.target.value })} placeholder="var(--oh-accent)" />
            <input
            type="color"
            value={hexToRgb(fields.color) ? (fields.color.startsWith("#") ? fields.color : `#${fields.color}`) : "var(--oh-accent)"}
    ...
    if (tool === "edit-feature") {
        ...
        return (
            ...
            {shown.map(({ feature, index }) => {
                ...
                return (
                    <div key={index} style={{ background: "var(--oh-plate-2)", border: "1px solid var(--oh-line)", borderRadius: 8, padding: "0.5rem 0.6rem" }}>
                    <div style={{ alignItems: "center", display: "flex", justifyContent: "space-between" }}>
    ...
    if (tool === "events") {
        ...
        return (
            ...
            {shown.map((event) => {
                ...
                return (
                    <div key={event.id} style={{ background: "var(--oh-plate-2)", border: "1px solid var(--oh-line)", borderRadius: 8, padding: "0.5rem 0.6rem" }}>
                    <div style={{ alignItems: "center", display: "flex", gap: "0.4rem", justifyContent: "space-between" }}>
                    <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: "var(--oh-t-xs)", fontWeight: 700, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{event.title || "(untitled)"}</div>
                    <div style={{ color: "var(--oh-text-dim)", fontSize: "var(--oh-t-2xs)" }}>{event.date}</div>
                    </div>
                    <div style={{ display: "flex", flexShrink: 0, gap: "0.3rem" }}>
