...
const withAlpha = (hex, alpha) => {
  ...
  if (!m) return `rgba(184,146,42,${alpha})`;
...
const CountryPickerMap = ({
  ...
  useEffect(() => {
    // The same three settings the map editor documents for this exact
    // geometry (see Editor/OlMap.jsx): rasterize the ~3,600 regions once per
    // view instead of re-drawing every polygon on every interaction frame,
    // and never draw the world twice side by side.
    ...
    // A style object per VISUAL STATE, not per feature: this function runs for
    // every one of the ~3,600 regions on every render pass, and it used to
    // allocate a Style, a Fill and a Stroke each time. The state a region can
    // be in is one of a handful, so they are built once and handed out.
    const styleCache = new Map();
    const cachedStyle = (key, build) => {
      let style = styleCache.get(key);
      if (!style) { style = build(); styleCache.set(key, style); }
      return style;
    };
    const styleFn = (feature) => {
      if (modeRef.current === "regions") {
        ...
        return cachedStyle(`r|${isSelected}|${isHovered}|${selectionColorRef.current}`, () => new Style({
          fill: new Fill({
            color: isSelected
              ? withAlpha(selectionColorRef.current, 0.6)
              : isHovered ? "var(--oh-accent-soft)" : "rgba(60,65,80,0.3)",
          }),
          stroke: new Stroke({
      ...
      if (!isPlayable) {
        return cachedStyle("unplayable", () => new Style({
      ...
      // Keyed on the colour, so one style per country rather than per feature.
      return cachedStyle(`c|${isHovered}|${isHovered ? "" : code}`, () => new Style({
        fill: new Fill({
          color: isHovered ? "var(--oh-accent-soft)" : codeToColor(code),
        }),
        stroke: new Stroke({
          color: isHovered
            ? "var(--oh-accent-soft)"
    ...
    olMap.on("pointermove", (evt) => {
      // Nothing to hover while the map is being dragged, and the hit test is
      // restricted to the region layer — it used to query every layer,
      // basemap tiles included, on every mouse move.
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
      ...
      >
        {filteredOptions.slice(0, query.trim() ? 30 : 12).map((c) => (
          ...
          >
            <span aria-hidden="true" style={{ fontSize: "var(--oh-t-md)", width: "1.5rem" }}>
        ...
        {filteredOptions.length > (query.trim() ? 30 : 12) && (
          <div style={{ color: "var(--oh-text-dim)", fontSize: "var(--oh-t-xs)", padding: "0.3rem", textAlign: "center" }}>
