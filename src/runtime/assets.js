...
import shippedCountryNames from "./countryNames.js";
...
export const loadCountryNames = async ({ force = false } = {}) => {
  ...
  const promise = (async () => {
    try {
      // The names come from a shipped list, not from the tiles. Reading them out
      // of tile zero cost a 62 MB download before the first turn, and the ten
      // modules that need this list — correspondence, naturalisation, the invitee
      // resolver, the translator, the pickers — want the NAMES, never the
      // geometry. The list is extracted from those very tiles by
...
export const loadRegionCatalog = async ({ force = false } = {}) => {
  ...
  const promise = (async () => {
    try {
      ...
      // Territory the GAME adds on top of its scenario (world.extraRegions —
      // the Vatican enclave, for one): named here so the model and the
      // player's region list can talk about it like any other region.
      try {
        const world = await readJson(JSON_URLS.world, { defaultValue: {} });
        for (const feature of normalizeExtraRegions(world?.extraRegions).features) {
          const { id, name, gid0 } = feature.properties;
          seen.set(id, { country: "", countryCode: gid0 || "", id, name });
        }
      } catch {
        // A world that fails to read adds nothing; the stock catalog stands.
      }
