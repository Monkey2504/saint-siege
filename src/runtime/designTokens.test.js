...
test("interface source strings are English, so the translator can reach every player", () => {
  const offenders = [];
  for (const file of files) {
    ...
    lines.forEach((line, i) => {
      ...
      // Accented letters inside a JSX text node or a quoted attribute/string.
      if (/[àâçéèêëîïôûùüÿœ]/i.test(line) && /(>[^<]*[àâçéèêëîïôûùüÿœ][^<]*<|"[^"]*[àâçéèêëîïôûùüÿœ][^"]*")/i.test(line)) {
        offenders.push(`${rel(file)}:${i + 1}: ${line.trim().slice(0, 90)}`);
      }
    });
  }
  assert.deepEqual(offenders, [], `non-English source strings:\n${offenders.join("\n")}`);
});
