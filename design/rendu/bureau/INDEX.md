# Rendu réel — bureau (1440×900)

Capturé le 2026-09-10 12:56 · commit b8e017f · source http://localhost:4173

Ces images sont le rendu réel du jeu dans un navigateur, pas une maquette.
Une session sans navigateur (Claude Design) doit les lire avant de juger le graphisme.

## Écrans

- `01-portail.png` — capturé, 409 Ko
- `02-cle-api.png` — capturé, 94 Ko
- `03-accueil.jpg` — capturé, 246 Ko
- `04-bibliotheque-parties.jpg` — capturé, 87 Ko
- `05-bibliotheque-scenarios.jpg` — capturé, 76 Ko
- `06-habemus-papam.png` — capturé, 211 Ko
- `07-courrier.png` — capturé, 76 Ko
- `08-bulletin.png` — capturé, 102 Ko
- `09-college.png` — capturé, 378 Ko
- `10-conseiller.png` — capturé, 42 Ko
- `11-comptes.png` — capturé, 176 Ko

Total du lot : 1898 Ko.

## Polices

- `--oh-font-display` → Bricolage Grotesque — réellement dessinée
- `--oh-font-body` → Archivo — réellement dessinée
- `--oh-font-label` → Archivo — réellement dessinée
- `--oh-font-data` → Archivo — réellement dessinée
- `--oh-font-serif` → Newsreader — réellement dessinée

Toutes les polices annoncées par les tokens sont réellement dessinées ici.

## Anomalies relevées pendant la capture

- réseau : www.googletagmanager.com
- réseau : fonts.googleapis.com
- réseau : open-historia-registry.nichojkrol.workers.dev

